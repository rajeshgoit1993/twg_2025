$(document).on("change",".propertysource",function() {
    ...
    if(propertysource=='manual') {
        ...
        $(this).parent().siblings(".similar_more").css('display', 'block');    
        ...
        $(this).parent().siblings(".similar_more").css('display', 'none');
        $(this).parent().siblings(".selectpropertystar").css('display','none')
        $(this).parent().siblings(".selectproperty").css('display','none')
        $(this).parent().siblings(".propertyname").css('display','none')
        $(this).parent().siblings(".selectpropertynamestar").css('display','none')
...
function fetch_hotel_name(city_name,propertytype,hotel_class) {
    $.ajax({
        ...
        success: function (data) {
            hotel_class.html("").html("<option value='0' selected='true' disabled='disabled'>Select Hotel</option>" + data)
        },
        error: function (data) {
        }
    });
...
$(document).on("keyup",".package_code",function() {

    var package_code=$(this).val()
    $.ajax({
        type: 'get',
        data: { package_code: package_code},
        url: APP_URL + "/check_package_code_availibility",
        success: function (data) {
        $(".package_code_availibility").html('').html(data)
        console.log(data)
        },
        error: function (data) {
        }
