...
<!-- /.box-header -->
<div class="box-body">
  <a href="{{URL::to('/hotel_image_location/'.Request::segment(3))}}" class="btn btn-success"><i class="glyphicon glyphicon-arrow-left"> </i> Back</a>
 
<h3 class="box-title">Add Photos From Gallery <span style="font-size: 14px">( {{$country_name}} &rarr; {{$city_name}} &rarr; {{CustomHelpers::get_hotel_title(Request::segment(3))}})</span></h3>


<form action="{{URL::to('/hotel_image_save/'.Request::segment(2).'/'.Request::segment(3).'/'.Request::segment(4))}}" method="post" >
...
<img src="{{URL::to('/').'/public/'.$datavalue->image_path }}" style="max-width: 100%;height: 200px"><br>
 ...
 <input type="radio" name="image_from_gallery" value="{{$datavalue->id}}" @if(CustomHelpers::get_data_conditions_hotel($datavalue->id)==Request::segment(2)) checked  @endif> 
