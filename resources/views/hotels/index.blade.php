            ...
            <div id="main">
               <div class="row">
                  <div class="col-sm-4 col-md-3">
                     ...
                     <h4 class="search-results-title"><i class="soap-icon-search"></i><b>{{count($Hotels)}}</b> Hotels found.</h4>
                        ...
                        {{--  {{dump($Hotels)}}  --}}
                            ...
                            <article class="box">
                                <figure class="col-sm-5 col-md-4">
                                    <a  title="" href="{{URL::to('/hoteldetail/'.Crypt::encrypt(['id'=>$hotel->id]))}}">
                                        <img width="270" height="160" alt="" src="{{ url('/public'.CustomHelpers::get_first_image($hotel->id,'rt_hotel_uploads','image_path','hotel_id')) }}">   
                                    </a>
                                    </figure>
                                <div class="details col-sm-7 col-md-8">
                                        ...
                                        <div>
                                            <h4 class="box-title">
                                                ...
                                                <small>
                                                    ...
                                                    {{$hotel->city}}, {{CustomHelpers::getTableRecordById($hotel->state,'subregions','name') }}
                                                </small>
                                            </h4>
                                    ...
                                    <div>
                                        @php
                                        $facilities = CustomHelpers::comma_separated_to_array($hotel->facilities);
                                        $roomsAval = CustomHelpers::countRows($hotel->id,'rt_rooms','id','assignedHotelname');
                                        @endphp
                                        <div class="amenities amenitiesHome">
                                            @foreach($facilities as $key=>$facilitie)
                                            <span class="amn" data-toggle="tooltip"  title="{{ CustomHelpers::getTableRecordById($facilitie,'rt_hotel_facilities','name') }}"><i class="fa circle {{ CustomHelpers::getTableRecordById($facilitie,'rt_hotel_facilities','icon') }}"></i> {{ CustomHelpers::getTableRecordById($facilitie,'rt_hotel_facilities','name') }} </span>
                                            @endforeach  
                                            ...
                                            <p><i class="fa fa-bed "></i> 
                                                {{--  {{ CustomHelpers::countRows($hotel->id,'rt_rooms','id','assignedHotelname') }}  --}}
                                                Rooms Available</p>
                                            @php
                                            }else{
                                            @endphp
                                        ...
                                        <div>
                                            ...
                                            @php
                                                if($roomsAval > 0){
                                            @endphp
                                                <span class="price"><small>AVG/NIGHT</small> {{ CustomHelpers::getRoomPriceByHotel($hotel->id) }}</span>
                                                <a class="button btn-small full-width text-center" title="" href="{{URL::to('/hoteldetail/'.Crypt::encrypt(['id'=>$hotel->id]))}}">SELECT</a>
                                            @php
                                            }else{
                                            @endphp
                                                <button class="button btn-small full-width text-center" title="" href="#">Sold Out</button>
                                            @php
                                            }
                                            @endphp
