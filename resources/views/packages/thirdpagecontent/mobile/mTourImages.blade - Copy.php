			...
			<div class="mTourImgBox mTourImg-animation-container"> <!--Mobile Tour Image Animation-pagethree.css-->
					...
					@foreach($images as $img)
						@if(CustomHelpers::get_image_gallery($img->gallery_id,'image_main')!="0")
							@if($ab==1)
							<img src="{{ CustomHelpers::get_image_gallery($img->gallery_id,'image_main') }}" alt="TourImage">
						@endif
				...
				<div class="mBannerCont">
					<a class="mGoToPreviousPage" href="{{ URL::previous() }}"></a>
					@php
					// Check if package_category is not null or empty
					if (!empty($details->package_category)) {
						$theme = unserialize($details->package_category);
					...
					@endphp
						<!--<div class="mTheme_position" style="display: none;">
						<ul>
							@for($i=0;$i<$theme_count;$i++)
								@if($i<="2")
									<li>{{ $theme[$i] }}</li>
