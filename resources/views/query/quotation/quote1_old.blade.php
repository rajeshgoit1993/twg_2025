						...
						<div class="panelContent">
							<div class="row">
								<div class="col-md-4 apndBtm10">
									...
									<input type="text" class="form-control textCapitalize" name="custom_package_name" id="custom_package_name" placeholder="Enter Package Name" value="@if(is_numeric($data->packageId)) {{CustomHelpers::get_package_name($data->packageId)}} @endif" />
								</div>
...
<div class="col-md-4">
                                <div class="form-group">
                                      ...
                                      <select name="duration" id="package_durations" class="form-control val" required>
                                        ...
                                        @for($i = 1; $i <= 30; $i++)
                                          <option value="{{ $i }}">                                              
                                              {{ $i }} {{ $i == 1 ? 'Night' : 'Nights' }} / {{ $i + 1 }} {{ ($i + 1) == 1 ? 'Day' : 'Days' }}
                                          </option>
                                      @endfor
                                      </select>
                                    </div>
              ...
              <option value="2" @if($data->package_tour_type==2) selected @endif>Customized Tour </option>
                                    
              <option value="3" @if($data->package_tour_type==3) selected @endif>Group Tour </option>
								...
								<div class="col-md-3 apndBtm10">
									...
									<input type="date" name="tour_date" id="tour_date" class="form-control tour_date" value="{{$data->date_arrival}}" placeholder="Tour Date" />
								</div>
                              ...
                              <?php 
                               ...
                               $a = 0; 
 $quote_continent = unserialize($data->package_continent);

                              ?>
                              @foreach($quote_continent ?? [] as $row => $col)
                            <?php

              $days_duration = $data->package_duration ?? ''; 
     
     $quote_country = unserialize($data->package_country); 
     $quote_state = unserialize($data->package_state); 
     $quote_city = unserialize($data->package_city); 

     $quote_day = unserialize($data->package_days);               
  if (isset($quote_continent[$row])) {
    $countries = DB::table('countries')->where('continent_id', $quote_continent[$row])->get();
  }
  else
  {
...
$states = DB::table('states')->where('country_id', $quote_country[$row])->get();
$cities = DB::table('city')->where('state_id', $quote_state[$row])->get();
 ...
 $days = $quote_day[$row] ?? '';          
    ...
    <option value="{{ $cont->id }}" 
        @if(isset($quote_continent[$row]) && $quote_continent[$row] == $cont->id) 
            selected 
        @endif>
        {{ $cont->continent_name }}
    </option>
@endforeach
                                      ...
                                      <div class="col-md-2">
                                       ...
                                       @foreach($countries as $country)
                                              <option value="{{ $country->id }}" @if($quote_country[$row]==$country->id) selected @endif>{{ $country->name }}</option>
                                            @endforeach
                                          </select>
                                        </div>
                                      </div>
                                        ...
                                        <div class="form-group">
                                          ...
                                          <select name="state[{{$a}}]" id="package_dest_state{{$a}}" class="form-control package_dest_state" >
                                           @foreach($states as $state)
                                              <option value="{{ $state->id }}" @if($quote_state[$row]==$state->id) selected @endif>{{ $state->name }}</option>
                                            @endforeach 
                                          </select>
                                        </div>
                                      </div>
                                        ...
                                        <div class="form-group">
                                          ...
                                          <select name="city[{{$a}}]" id="package_dest_city{{$a}}" class="package_dest_cities form-control city_package_dest_countries min-select" >
                                             @foreach($cities as $city)
                                              <option value="{{ $city->id }}" @if($quote_city[$row]==$city->id) selected @endif>{{ $city->name }}</option>
                                            @endforeach 
                                          </select>
                                        </div>
                                      </div>
								...
								<tbody>
									...
									<tr>
										...
										<td class="makeflex">
											...
											<option coupon_id="{{$markup_pro->id}}"  value="{{$markup_pro->value}}" >{{$markup_pro->coupon_name}}</option>
											@endforeach
										</select>
												...
												<div class="col-md-3 apndBtm20">
													...
													<select name="flight[origin]" class="form-control origin">
														...
														<?php $val=$iata->iata_name.' '.'('.$iata->iata_code.')'; ?>
															<option value="{{$val}}">{{$iata->iata_name}} ({{$iata->iata_code}}) </option>
														@endforeach
													</select>
												</div>
												<div class="col-md-3 apndBtm20">
													...
													<div class="clearfix"></div>
														<select name="flight[dhours]" class="form-control dhours" style="max-width: 49%;display: inline-block;">
															...
															@for($i=1; $i<=24; $i++)
																<option value="{{ $i }}">{{ $i }} {{ \Illuminate\Support\Str::plural('hour', $i) }}</option>
															@endfor
														</select>
														<select name="flight[dmins]" class="form-control dmins" style="max-width: 49%;display: inline-block;">
															...
															@for($i = 0; $i <= 59; $i++)
																<option value="{{ $i }}">{{ $i === 0 ? '00' : $i }} {{ \Illuminate\Support\Str::plural('minute', $i) }}</option>
															@endfor
														</select>
												</div>
												<div class="col-md-3 apndBtm20">
												...
												<div class="col-md-3 apndBtm20">
													...
													<div class="clearfix"></div>
														...
														<select name="flight[duration_dmins]" class="form-control duration_min" style="max-width: 49%;display: inline-block;">
															<option value="" selected disabled>Minutes</option>
															@for($i = 0; $i <= 59; $i++)
															    <option value="{{ $i }}">{{ $i === 0 ? '00' : $i }} {{ $i === 1 ? 'minute' : 'minutes' }}</option>
															@endfor
														</select>
												</div>
												<div class="col-md-3 apndBtm20">
									...
									<div class="table-responsive">
										...
										<div class="item-container day1">
											<div class="dayTitle">DAY {{ $j }}
												<input type="text" name="dayItinerary[day{{$j}}][title]" placeholder="Day Title">
											</div>
											<div class="form-group">
										...
										<tbody>
											<tr>
												<td>
													...
													<i class="fa @if($loged_user->lock_header_email==1) fa-lock @else fa-unlock @endif colorA1 fontSize14 lock_header_icon" aria-hidden="true"></i>
													<br>
													<span class="show_hide morePlus">More+</span>
													<br>
													<textarea name="quotation_header_extra" placeholder="Please state your Extra Quotation Header..." rows="6" class="form-control ckeditor" @if($loged_user->lock_header_email==1) readonly @endif> {!! $loged_user->signature_header !!} </textarea>
												</td>
											</tr>
											<tr>
												<td>
													<label for="webHeader" class="webHeader">Web header <span class="requiredcolor">*</span></label> 
													<i class="fa  @if($loged_user->lock_header==1) fa-lock @else fa-unlock @endif colorA1 fontSize14 lock_header_icon" aria-hidden="true">
													</i>
													<br>
													...
													<div>
														...
														<select name="quotation_header" class="select2 form-control" @if($loged_user->lock_header==1) style="cursor:not-allowed" disabled @endif>
															@foreach($quotation_header as $pol)
																<option value="{{$pol->id}}" @if($loged_user->quotation_header==$pol->id) selected @endif>{{$pol->header}} </option>
															@endforeach
														</select>
													</div>
												</td>
										...
										<tbody>
											<tr>
												<td>
													...
													<i class="fa  @if($loged_user->lock_footer_email==1) fa-lock @else fa-unlock @endif colorA1 fontSize14 lock_header_icon" aria-hidden="true">
													</i>
													<br>
													<span class="show_hide morePlus">More+</span>
													<br>
													<textarea name="quotation_footer_extra" placeholder="Please state your Extra Quotation Footer..." rows="6" class="form-control ckeditor" @if($loged_user->lock_footer_email==1) readonly @endif> {!! $loged_user->signature !!} </textarea>
												</td>
											</tr>
											<tr>
												<td>
													<label for="webFooter" class="webFooter">Web footer <span class="requiredcolor">*</span></label>
													<i class="fa  @if($loged_user->lock_footer==1) fa-lock @else fa-unlock @endif colorA1 fontSize14 lock_header_icon" aria-hidden="true"></i>
													<br>
													<div>
														<input type="hidden" name="quotation_footer" class="quotation_footer" value="{{$loged_user->quotation_footer}}">
														<select name="" class="select2 form-control" @if($loged_user->lock_footer==1) style="cursor:not-allowed" disabled @endif>
															@foreach($quotation_footer as $pol)
																<option value="{{$pol->id}}" @if($loged_user->quotation_footer==$pol->id) selected @endif>{{$pol->footer}} </option>
															@endforeach
														</select>
													</div>
												</td>
