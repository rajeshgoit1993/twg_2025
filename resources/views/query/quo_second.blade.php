...
<section class="content">
    ...
    <button onclick="window.history.back()" class="btn btn-success" style="margin-right: 20px">
    ...
    <span class="pfwmt font-size18">Create Quotation</span>
                ...
                <div class="panel-body" style="padding: 0px">
                            ...
                            <div class="row">
                                <div class="col-md-4 appendBottom10">
                                    ...
                                    @if(is_numeric($data->packageId))
                                        ...
                                        <input type="hidden" id="is_valid_package" value="1">
                                    @else
                                        ...
                                        <input type="hidden" id="is_valid_package" value="0">
                                        <div class="alert alert-warning appendTop10">
                                            <i class="fa fa-exclamation-triangle"></i> This is a custom package. Please import from reference below.
                                        </div>
                                    @endif
                                </div>
                                <div class="col-md-4 appendBottom10">
                    ...
                    <div style="padding: 0px 10px">
                        <form action="{{URL::to('/copy_reference/'.$data->id)}}" method="post" id="importForm">
                        {{csrf_field()}} 
                    ...
                    <!-- Main Form Section -->
                    <div style="padding: 10px" class="@if(!is_numeric($data->packageId)) disabled-section @endif" id="mainFormSection">
                        <form action="{{URL::to('/submit_quotation')}}" method="post" id="quotationForm">
                            {{csrf_field()}}
                            <input type="hidden" name="query_id" value="{{$data->id}}">
                            
                            <!-- Your quotation form fields would go here -->
                            <div class="row appendTop20">
                                <div class="col-md-12">
                                    <button type="submit" class="btn btn-primary pull-right">Create Quotation</button>
                                </div>
                            </div>
                        </form>
                    </div>
...
$(document).ready(function() {
    // Check package validity on page load
    var isValidPackage = $('#is_valid_package').val();
    
    // If package is not valid (custom package), disable main form
    if(isValidPackage === '0') {
        $('#mainFormSection').addClass('disabled-section');
    }
    ...
    $('#quotationForm').submit(function(e) {
        var isValid = $('#is_valid_package').val();
        if (isValid === '0') {
            e.preventDefault();
            alert('Please import a valid package reference before creating quotation.');
            return false;
        }
        return true;
    });
