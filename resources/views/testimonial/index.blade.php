            ...
            <div class="col-md-12">
                <div class="box">
                    ...
                    <div class="box-body">
                        ...
                        @if(Sentinel::check() && (Sentinel::getUser()->inRole('administrator') || Sentinel::getUser()->inRole('supervisor') || Sentinel::getUser()->inRole('super_admin')))
                            <div class="add">
                                <a href="{{ URL::to('/add_testimonial') }}" class="btn btn-success">
                                      ...
                                      <tr>
                                        ...
                                        <td>
                                          ...
                                          <div class="list-item-image relativeCont">
                                            @if($testimonialImg && $testimonialImg->type != "")
                                              @if($testimonialImg->type == 'image')
                                                @php
                                                  ...
                                                  $imageSrc = (!empty($testimonialImg->image_thumb) && file_exists($imagePath)) 
                                                              ...
                                                              : $defaultImage;
                                                @endphp
                                                <img src="{{ asset('public/' . $imageSrc) }}" alt="img">
                                              @else
                                                @php
                                                ...
                                                @if($videoExists)
                                                  <p class="videoSize">{{ $videoSize }} MB</p>
                                                  <video controls>
                                                    <source src="{{ url('public/uploads/testimonial/'.$testimonialImg->image_or_video) }}" type="video/mp4">
                                                    <source src="{{ url('public/uploads/testimonial/'.$testimonialImg->image_or_video) }}" type="video/ogg">
                                                    Your browser does not support the video tag.
                                                  </video>
                                                @else
                                                  <img src="{{ asset('public/' . $defaultImage) }}" alt="Default Image">
                                                @endif
                                              @endif
                                            @else
                                              @php
                                        ...
                                        <td>{{ $single->c_name }}</td>
                                        <td>{{ $single->c_email }}</td>
                                        <td>{{ $single->c_mobile }}</td>
                                        <td>{{ $single->c_country }}</td>
                                        <td>{{ substr($single->c_exp, 0, 50) }}</td>
                                        <td>{{ $single->c_rating }} star</td>
                                        <td>
                                            <button type="button" class="btn btn-info btn-sm uploads" id="{{ $single->id }}">Uploads</button>
                                        </td>
                                        <td>
                                            @if($single->status == 1)
                                        ...
                                        <td>
                                            <!-- <form action="{{URL::to('/delete_testimonial/'.$single->id)}}" onsubmit="return confirm('Do you really want to delete this.?');" method="POST">
                                              ...
                                              @if(Sentinel::getUser()->inRole('administrator') || Sentinel::getUser()->inRole('supervisor') || Sentinel::getUser()->inRole('super_admin'))
                                                <a class="btn btn-default btn-xcrud btn btn-warning" href="{{ URL::to('/edit_testimonial/'.$single->id) }}">
                                                <i class="fa fa-edit"> Edit</i>
                                                </a>
                                                <input type="hidden" value="{{url('/user-review/'.CustomHelpers::custom_encrypt($single->id))}}" id="copy{{CustomHelpers::custom_encrypt($single->id)}}">
                                                <button type="button" class="btn btn-success link" link="copy{{CustomHelpers::custom_encrypt($single->id)}}">Copy Link</button>
                                                @endif
                                              @endif
                                              @if(Sentinel::check())
                                            ...
                                            @if(Sentinel::check() 
                                              ...
                                              <!-- edit -->
                                              <a href="{{ URL::to('/edit_testimonial/'.$single->id) }}">
                                                <button class="btn btn-warning btn-sm"><i class="fa fa-edit"></i> Edit</button>
                                              </a>

                                              <!-- copy link -->
                                              <input type="hidden" value="{{ url('/user-review/'.CustomHelpers::custom_encrypt($single->id))}}" id="copy{{ CustomHelpers::custom_encrypt($single->id) }}">
                                              <button type="button" class="btn btn-success btn-sm link" link="copy{{ CustomHelpers::custom_encrypt($single->id) }}">Copy Link</button>
                                            @endif

                                            <!-- delete -->
                                            <form action="{{ URL::to('/delete_testimonial/'.$single->id) }}" onsubmit="return confirm('Are you sure, you want to delete this?');" method="POST">
                                              {{ csrf_field() }}
                                              <input type="hidden" name="id" value=""/>
                                              @if(Sentinel::check() 
                                                && (Sentinel::getUser()->inRole('administrator') 
