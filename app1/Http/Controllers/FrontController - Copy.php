...
class FrontController extends Controller 
{
    ...
    public function packages1($id, Request $request) 
    {
        $date = $request->has('datepicker') ? $request->datepicker : null;
        $theme_name = '';

        // Process the destination search
        $destination_search_first = explode("-", $id);
        $remove_array = ['tour', 'packages'];
        $destination_search = array_diff($destination_search_first, $remove_array);
        $destination_search = implode(" ", $destination_search);
        ...
        // Get the last two values from the exploded array
        $array = explode("-", $id);
        $first_value = end($array);
        $second_value = prev($array);

        // Handle package and tour conditions
        if ($first_value != "packages" && $second_value != "tour") {
            // Decrypt and fetch package data
            $data_id = CustomHelpers::custom_decrypt($request->package_id);
            $Packages = Packages::findOrFail($data_id);
            ...
            // Fetch related data
            $images = PackageUploads::where('package_id', $data_id)->get();
            $daywise = unserialize($Packages->day_itinerary);
            $city = unserialize($Packages->city);
            $country = unserialize($Packages->country);
            $continent = unserialize($Packages->continent);
            ...
            if ($status == 1 || in_array(Sentinel::getUser()->roles()->first()->slug, ['super_admin', 'administrator', 'supervisor', 'employee'])) {
        ...
        // Search conditions for packages
        $search_conditions = [['status', '=', '1']];

        if (!empty($destination_search)) {
            $search_conditions[] = ['continent', 'like', '%' . $destination_search . '%'];
            $search_conditions[] = ['country', 'like', '%' . $destination_search . '%'];
            $search_conditions[] = ['state', 'like', '%' . $destination_search . '%'];
            $search_conditions[] = ['city', 'like', '%' . $destination_search . '%'];
        }

        // Get all matching packages data
        $data = DB::table('rt_packages')
            ->where($search_conditions)
            ->orderBy('sort', 'ASC')
            ->get();

        // Get package IDs for the destination search
        ...
        // Get top 3 packages based on the destination search
        ...
        // Get SEO data for the destination search
    ...
    {
        $date = $request->datepicker;
        $destination_search = $this->generateDestinationSearch($id);

        // Try to find the package by ID if it's not a standard 'tour' or 'packages'
        if ($this->isValidPackageRequest($id)) {
            return $this->getPackageDetails($request, $id);
        }

        // If package is not valid, proceed with searching packages
        return $this->searchPackagesByDestination($destination_search, $date);
    }

    private function generateDestinationSearch($id)
    {
        $destination_search_first = explode("-", $id);
        $remove_array = ['tour', 'packages'];
        $destination_search = array_diff($destination_search_first, $remove_array);
        $destination_search = implode(" ", $destination_search);
        return str_replace(' And ', ' and ', ucwords($destination_search));
    }

    private function isValidPackageRequest($id)
    {
        $array = explode("-", $id);
        $first_value = end($array);
        $second_value = prev($array);
        return $first_value != "packages" && $second_value != "tour";
    }

    private function getPackageDetails($request, $id)
    {
        $data_id = CustomHelpers::custom_decrypt($request->package_id);
        $Packages = Packages::findOrFail($data_id);
        if ($Packages->onrequest == 1) {
        ...
        if ($status == 1) {
            return view('packages.pagethree', compact(
                'Packages', 'price', 'images', 'daywise', 'city', 'country', 'continent', 'state', 'data_id', 'hoteldata', 'icon_data', 'id'
            ));
        } else {
            return $this->checkUserRoleAndRedirect($Packages);
        }
    }

    private function checkUserRoleAndRedirect($Packages)
    {
        if (Sentinel::check()) {
            $userRole = Sentinel::getUser()->roles()->first()->slug;
            $allowedRoles = ['super_admin', 'administrator', 'supervisor', 'employee'];
            if (in_array($userRole, $allowedRoles)) {
                return view('packages.pagethree', compact('Packages'));
            }
        }
        return redirect('/');
    }

    private function searchPackagesByDestination($destination_search, $date)
    {
        $data = DB::table('rt_packages')->where([['continent', 'like', '%' . $destination_search . '%'], ['status', '=', '1']])
        ...
        $packages = $data->limit(3);
        $data1 = DB::table('rt_packages_seo')->where([['destination', 'like', '%' . $destination_search . '%']])->get();
        $icon_data = Icons::all();

        return view('packages.pagetwo', compact('data', 'data_ids', 'destination_search', 'icon_data', 'data1', 'packages', 'date'));
    }*/
    ...
    /*public function search_theme(Request $request)
    {
        ...
        // Check if we found any themes or not
        if (empty($getdata)) {
            // If no themes were found, perform a continent search
            $data = DB::table('rt_packages')
                ->where([['continent', 'like', '%' . $search_theme . '%'], ['status', '=', '1']])
                ->get();
            ...
            // Extract package categories from the continent search data
            foreach ($data as $data_theme) :
                $data_value_theme = unserialize($data_theme->package_category);
                $count = count($data_value_theme);
            ...
            // Split and remove duplicate values from the continent search
            $theme_front_data = explode(",", $getdata);
            $theme_front_data = array_unique($theme_front_data);
    ...
    /*public function search_theme(Request $request)
    {
        ...
        // Fallback: If "All" is selected, fetch all active packages
        $query = DB::table('rt_packages')->where('status', '=', '1');
        if ($search_theme !== '' && $search_theme !== null) {
            $fields = ['city', 'country', 'state', 'continent'];
            $query->where(function ($q) use ($fields, $search_theme) {
                foreach ($fields as $field) {
                    $q->orWhere($field, 'like', '%' . $search_theme . '%');
                }
            });
        }

        // Fetch data and extract themes
        $data = $query->get();
    ...
    private function generateOptions($themes)
    {
        $options = "<option value=''>Select Theme</option>";
        foreach ($themes as $theme) {
            $options .= "<option value='$theme'>" . e($theme) . "</option>";
        }
        return $options;
    ...
    // load more packages data (page two)
