...
class QueryController extends Controller 
{
   ...
   public function save_quote(Request $request)
   {
    ...
    public function quotation_details_first($id) {
        ...
        // Retrieve data for each quotation option
        ...
        $this->clearQuotationSessionData($unique_code);

        // Set session data only if the data exists
        if ($data1) {
            Session::set($unique_code . 'quote1_id', $data1->id);
        }
        if ($data2) {
        ...
        // Handle views based on the retrieved data and user roles
        if (Sentinel::check()) {
            $userRoleSlug = Sentinel::getUser()->roles()->first()->slug;

            if (in_array($userRoleSlug, ['super_admin', 'administrator', 'employee'])) {
                return $this->getQuotationViewBasedOnStatus($data1, $data2, $data3, $data4);
            }
        }

        return $this->getQuotationViewForCustomer($data1, $data2, $data3, $data4);
    }
    ...
    private function getQuotationViewBasedOnStatus($data1, $data2, $data3, $data4) {
        // Return views based on the status of the data objects
