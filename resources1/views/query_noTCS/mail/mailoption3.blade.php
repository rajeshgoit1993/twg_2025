...
<img src="http://imgak.mmtcdn.com/holidays/images/quotemanager/rupee.png"> {{CustomHelpers::get_quotation_grandtotal($data->option3_price,'adult')}} <i>for <?php 
$total_traveler="0";
...
<!-- next -->
@if($data->option3_transport=="Flight")
<?php
$flight_detail=unserialize($data->option3_flight);
...
<?php
$acco=unserialize($data->option3_accommodation);
...
@php
$itinerary_data=unserialize($data->option3_dayItinerary);
$day="1";
@endphp
...
<td style="padding: 15px 36px;background: #fff;border: 1px solid #bdbdbd;line-height: 22px;font-size: 14px;color: #636363;">
	...
	@if(empty($data->option3_package_visa) || $data->option3_package_visa=="N;")

	@else

	<?php

   $v_policy=unserialize($data->option3_package_visa);
   ?> 
...
@endif
<!-- next -->
@if(empty($data->option3_package_payment) || $data->option3_package_payment=="N;")
...
$p_policy=unserialize($data->option3_package_payment);
?> 
...
@endif
<!-- next -->
@if(empty($data->option3_package_can) || $data->option3_package_can=="N;")
...
$c_policy=unserialize($data->option3_package_can);
?> 
...
@endif
<!-- next -->
@if(empty($data->option3_package_impnotes) || $data->option3_package_impnotes=="N;")
...
$important_notes=unserialize($data->option3_package_impnotes);
?> 
