...
<section class="content">
	<div class="row">
		...
		<div class="box-body">
			<!--Add Lead Button-->
			@include('query.enquiryDetails.addLeadBtn')
			<!--Add Lead Button Ends-->
		...
		<tr>
			...
			<td>
				@if(is_numeric($query->packageId))
					<a href="{{url('/Holidays/'.str_slug(CustomHelpers::get_package_name($query->packageId)).'?package_id='.CustomHelpers::custom_encrypt($query->packageId))}}" target="_blank">{{CustomHelpers::get_package_name($query->packageId)}}</a>
				@else
				{{$query->packageId}}
				@endif
			</td>
			...
			<td><?php $newDate_flight = date("d M Y H:i", strtotime($query->created_at)); ?>
				{{$newDate_flight}}
				<br>
	...
	@include('query.enquiryDetails.viewLeadModal')
	<!---->
</section>
