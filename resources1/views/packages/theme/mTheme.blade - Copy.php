...
<div class="dThemeContBG">
	<div class="container">
		...
		<?php
			...
			$data=DB::table('rt_packages')->where([['continent', 'like', '%' . $destination_search . '%'],['status', '=', '1'],['package_category', 'like', '%' . $select_theme . '%']])->orwhere([['country', 'like', '%' . $destination_search . '%'],['status', '=', '1'],['package_category', 'like', '%' . $select_theme . '%']])->orwhere([['state', 'like', '%' . $destination_search . '%'],['status', '=', '1'],['package_category', 'like', '%' . $select_theme . '%']])->orwhere([['city', 'like', '%' . $destination_search . '%'],['status', '=', '1'],['package_category', 'like', '%' . $select_theme . '%']])->limit(6)->get();
		?>
		<div class="mThemeContnt">
			...
			@endphp
			<div class="col-xs-6 col-sm-4 custom_length_demostic" style="padding-right: 0;">
				<div class="mTourItem">
						...
						<div class="mTourPrcBox">
							@if($package->onrequest == 1 && $package->upcoming == 1)
								<div class="mTourPrcValueBox">
									<p class="mTourPrcValueReq"><span class="dTourDefaultCurency">&nbsp;</span>On Request</p>
								</div>
							@elseif($package->onrequest != 1 && $package->upcoming == 1)
								...
								@else
									<div class="mTourPrcValueBox">
										<p class="mTourPrcValue"><span class="dTourDefaultCurency">&nbsp;</span>{{ CustomHelpers::get_price($package->id) }}</p>
							...
							@elseif($package->onrequest == 1 && $package->upcoming != 1)
								...
								@else
									<div class="mTourPrcValueBox">
										<p class="mTourPrcValue"><span class="dTourDefaultCurency">&nbsp;</span>{{ CustomHelpers::get_up_price($package->id) }}</p>
